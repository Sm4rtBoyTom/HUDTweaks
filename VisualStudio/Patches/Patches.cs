    ...
    internal class Patches
    {
        ...
        internal static class RemoveAfflictionIcon
        {
            private static void Postfix(Panel_HUD __instance)
            {
                ...
                if (__instance != null)
                {
                    ...
                    if (Settings.instance.BuffPopups == true)
                    {
                        __instance.m_BuffNotificationDurationSeconds = 0;
                    }
                    else
                    {
                        __instance.m_BuffNotificationDurationSeconds = 6;
                    }

                    __instance.m_FeatUnlockedWidget.gameObject.SetActive(Settings.instance.feats);
                    if (!Settings.instance.AimMeter)
                    {
                        __instance.m_AimingStaminaBar.gameObject.SetActive(false);

                    }
                    if (!Settings.instance.SafehouseIcon)
                    {
        ...
        private static class RemoveAmmoIcon
        {
            private static void Postfix(EquipItemPopup __instance)
            {
                ...
                if (__instance != null)
                {
                    if (!Settings.instance.AmmoWidgets)
                    {
